...
import (
	...
	"github.com/diademnetwork/go-diadem/client/plasma_cash/eth/ethcontract"
...
func (c *EthPlasmaClientImpl) FetchWithdrews(startBlock, endBlock uint64) ([]*pctypes.PlasmaCashWithdrewEvent, error) {
	...
	for iterator.Next() {
		...
		localOwnerAddress, err := diadem.LocalAddressFromHexString(event.Owner.Hex())
		if err != nil {
			return nil, errors.Wrap(err, "failed to parse plasma coin withdraw owner's address")
		}
		localContractAddr, err := diadem.LocalAddressFromHexString(event.ContractAddress.Hex())
		if err != nil {
		...
		withdrawCoinEvents = append(withdrawCoinEvents, &pctypes.PlasmaCashWithdrewEvent{
			Slot:         event.Slot,
			Owner:        diadem.Address{ChainID: "eth", Local: localOwnerAddress}.MarshalPB(),
			Mode:         uint32(event.Mode),
			Uid:          &ltypes.BigUInt{Value: *diadem.NewBigUInt(event.Uid)},
			Denomination: &ltypes.BigUInt{Value: *diadem.NewBigUInt(event.Denomination)},
			Contract:     diadem.Address{ChainID: "eth", Local: localContractAddr}.MarshalPB(),
...
func (c *EthPlasmaClientImpl) FetchDeposits(startBlock, endBlock uint64) ([]*pctypes.PlasmaDepositEvent, error) {
	...
	for iterator.Next() {
		event := iterator.Event
		localFromAddr, err := diadem.LocalAddressFromHexString(event.From.Hex())
		if err != nil {
		...
		depositEvents = append(depositEvents, &pctypes.PlasmaDepositEvent{
			Slot:         event.Slot,
			DepositBlock: &ltypes.BigUInt{Value: *diadem.NewBigUInt(event.BlockNumber)},
			Denomination: &ltypes.BigUInt{Value: *diadem.NewBigUIntFromInt(1)}, // TODO: ev.Denomination
			From:         diadem.Address{ChainID: "eth", Local: localFromAddr}.MarshalPB(),
			Contract:     diadem.Address{ChainID: "eth", Local: localContractAddr}.MarshalPB(),
			Meta: &pctypes.PlasmaCashEventMeta{
				BlockNumber: event.Raw.BlockNumber,
				TxIndex:     uint64(event.Raw.TxIndex),
