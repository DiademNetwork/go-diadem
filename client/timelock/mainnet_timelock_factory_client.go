...
type MainnetTimelockFactoryClient struct {
	...
	contract  *diademTimelockFactory
	// Mainnet TimelockFactory contract address
	Address   common.Address
	TxTimeout time.Duration
}

func (c *MainnetTimelockFactoryClient) FetchTokenTimeLockCreationEvent(caller *client.Identity, startBlock, endBlock uint64) ([]*diademTimelockFactorydiademTimeLockCreated, error) {
	filterOpts := &bind.FilterOpts{
		Start: startBlock,
		End:   &endBlock,
	}

	var tokenTimeLockCreationEvents []*diademTimelockFactorydiademTimeLockCreated

	iterator, err := c.contract.FilterdiademTimeLockCreated(filterOpts)
	if err != nil {
...
func (c *MainnetTimelockFactoryClient) DeployTimelock(caller *client.Identity, beneficiary common.Address, validatorName string, pubKey string, amount *big.Int, duration *big.Int) (common.Address, error) {
	...
	it, err := c.contract.FilterdiademTimeLockCreated(filterOpts)
	if err != nil {
...
func ConnectToMainnetTimelockFactory(ethClient *ethclient.Client, address string) (*MainnetTimelockFactoryClient, error) {
	...
	contract, err := NewdiademTimelockFactory(contractAddr, ethClient)
	if err != nil {
		return nil, err
	}
