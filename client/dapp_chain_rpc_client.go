...
func (c *DAppChainRPCClient) GetNonce(signer auth.Signer) (uint64, error) {
	return c.GetNonce2(diadem.Address{}, signer)
}
...
func (c *DAppChainRPCClient) GetNonce2(caller diadem.Address, signer auth.Signer) (uint64, error) {
	...
	//       this is done to maintain compatibility with older diademchain builds.
	//       Newer diademchain builds will ignore the key param if the account param is set,
...
func (c *DAppChainRPCClient) CommitTx(signer auth.Signer, tx proto.Message) ([]byte, error) {
	return c.CommitTx2(diadem.Address{}, signer, tx)
}
...
func (c *DAppChainRPCClient) Resolve(name string) (diadem.Address, error) {
	...
	return diadem.ParseAddress(addrStr)
}
...
func (c *DAppChainRPCClient) CommitDeployTxWithValue(
	...
	msgBytes, err := proto.Marshal(&vm.MessageTx{
		From: from.MarshalPB(),
		To:   diadem.Address{}.MarshalPB(), // not used
		Data: deployTxBytes,
	})
	if err != nil {
		return nil, err
