...
func (c *GRPCAPIClient) staticCall(addr diadem.Address, input []byte, vmType vm.VMType) ([]byte, error) {
...
func (c *GRPCAPIClient) Resolve(name string) (diadem.Address, error) {
	...
	return diadem.UnmarshalAddressPB(resp.Address), nil
}

func (c *GRPCAPIClient) call(addr diadem.Address, input []byte, vmType vm.VMType, value *diadem.BigUInt) ([]byte, error) {
	if value == nil {
...
func (c *GRPCAPIClient) Call(addr diadem.Address, input []byte) ([]byte, error) {
	return c.call(addr, input, vm.VMType_PLUGIN, diadem.NewBigUIntFromInt(0))
}
...
func (c *GRPCAPIClient) ContractRecord(contractAddr diadem.Address) (*ContractRecord, error) {
	...
	return &ContractRecord{
		...
		ContractAddress: diadem.UnmarshalAddressPB(resp.ContractAddress),
		CreatorAddress:  diadem.UnmarshalAddressPB(resp.CreatorAddress),
	}, nil
}
...
func (c *GRPCContext) Message() Message {
	return Message{
		Sender: diadem.UnmarshalAddressPB(c.message.Sender),
	}
}
...
func MakeGRPCContext(conn *grpc.ClientConn, req *types.ContractCallRequest) *GRPCContext {
	return &GRPCContext{
		...
		message:      req.Message,
		block:        req.Block,
		contractAddr: diadem.UnmarshalAddressPB(req.ContractAddress),
	}
}
