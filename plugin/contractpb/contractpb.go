...
func (c *wrappedPluginStaticContext) Logger() *diadem.Logger {
	return c.logger
}
...
func (c *wrappedPluginStaticContext) HasPermissionFor(addr diadem.Address, token []byte, roles []string) (bool, []string) {
	found := false
	foundRoles := []string{}
...
func (c *wrappedPluginContext) GrantPermissionTo(addr diadem.Address, token []byte, role string) {
...
func (c *wrappedPluginContext) RevokePermissionFrom(addr diadem.Address, token []byte, role string) {
...
func rolePermKey(addr diadem.Address, token []byte, role string) []byte {
	...
	//       address twice!
	return []byte(fmt.Sprintf("%stoken:%s:role:%s", diadem.PermPrefix(addr), token, []byte(role)))
}
...
func setupLogger() {
	onceSetup.Do(func() {
		...
		logger = diadem.NewdiademLogger(level, dest)
	})
}
...
func CallEVM(ctx Context, addr diadem.Address, input []byte, output *[]byte) error {
	resp, err := ctx.CallEVM(addr, input, diadem.NewBigUIntFromInt(0))
	*output = resp
	return err
}
