...
func NewdiademLogger(diademLogLevel, dest string) *Logger {
	w := MakeFileLoggerWriter(diademLogLevel, dest)
	...
	return MakediademLogger(diademLogLevel, w, logTr)
}
...
func MakediademLogger(logLevel string, w io.Writer, tr func(w io.Writer) kitlog.Logger) *Logger {
	loggerFunc := func(w io.Writer) *Logger {
		...
		baseLogger := kitlog.With(tr(w), "module", "diadem")
		return &Logger{
...
func MakeFileLoggerWriter(diademLogLevel, dest string) io.Writer {
	...
	//			syslogLevel(diademLogLevel)|syslog.LOG_DAEMON, "diadem")
	//		if err != nil {
	//			log.Fatal(err)
	//		}
