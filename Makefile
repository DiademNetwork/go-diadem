PKG = github.com/diademnetwork/go-diadem
...
# This commit sha should match the one in diademchain repo
...
lint:
	...
	cd $(GOPATH)/src/github.com/diademnetwork/go-diadem
	@golangci-lint run | tee godiademreport
...
$(SSHA3_DIR):
	git clone -q https://github.com/diademnetwork/go-solidity-sha3.git $@

$(GETH_DIR):
	git clone -q https://github.com/diademnetwork/go-ethereum.git $@
